...
const DETAIL = {
  ...
  emirates: {
    title: 'Data Engineer Consultant', subtitle: 'Emirates NBD · via ValueLabs · Jan 2025 – Present', sparkMeta: 'Cluster: prod-de-jobs · Runtime: 14.3 LTS · Photon: Enabled',
    html: `<div class="detail-block"><div class="detail-meta-row"><span class="detail-badge primary">PySpark</span><span class="detail-badge primary">Kafka</span><span class="detail-badge primary">Airflow</span><span class="detail-badge">Oozie</span><span class="detail-badge">Oracle</span><span class="detail-badge">SAP HANA</span><span class="detail-badge">ODS</span><span class="detail-badge success">Claude Code</span></div><div class="nb-cell"><div class="nb-cell__cmd">%spark</div><div class="nb-cell__code"><span class="nb-comment">// Wholesale Banking ETL — Spark config</span><br>spark.conf.set("spark.sql.adaptive.enabled", "true")<br>spark.conf.set("spark.sql.adaptive.coalescingPartitions.enabled", "true")<br>spark.conf.set("spark.databricks.delta.optimizeWrite.enabled", "true")</div></div><ul class="detail-list"><li>Engineered an <strong>end-to-end data pipeline</strong> for Wholesale Banking customer deals using <strong>PySpark, Kafka, and ODS</strong>, standardising approvals and tracking deal expiries to deliver <strong>significant business cost savings</strong>.</li><li>Spearheaded the <strong>decommissioning of SAP HANA</strong> by migrating existing BDM workflows to PySpark for <strong>5 banking entities</strong>, eliminating SAP license costs and successfully automating <strong>~180 daily jobs</strong> loading to Oracle.</li><li>Designed a <strong>highly parameterised PySpark framework</strong> configured via simple YAML files, allowing the engineering team to replicate ETL jobs with <strong>zero defects rapidly</strong>.</li><li>Managed complex daily and monthly ETL loads aggregating multiple finance sources and pricing structures, leveraging <strong>Airflow, Oozie, and GenAI tools</strong> like Claude Code to accelerate deployment timelines <strong>without any critical issues</strong>.</li></ul></div>`
  },
  epam: {
    title: 'Senior Data Engineer', subtitle: 'EPAM Systems · Client: Canadian Tire · Dec 2021 – Jan 2025', sparkMeta: 'Target: Azure Cosmos DB · Query gain: 40% · Daily throughput: 100 GB',
    html: `<div class="detail-block"><div class="detail-meta-row"><span class="detail-badge primary">Azure Data Factory</span><span class="detail-badge primary">Cosmos DB</span><span class="detail-badge">Hive</span><span class="detail-badge">Delta Lake</span><span class="detail-badge">JSON</span></div><div class="nb-cell"><div class="nb-cell__cmd">%sql</div><div class="nb-cell__code"><span class="nb-comment">-- Hive → Delta → Cosmos DB migration</span><br>CREATE TABLE delta_silver.dealer_analytics<br>USING DELTA<br>LOCATION 's3://datalake/silver/dealer_analytics'<br>AS SELECT * FROM hive_enterprise.dealer_data<br>WHERE partition_date >= '2021-01-01'</div></div><ul class="detail-list"><li>Developed <strong>feature-based data pipelines</strong> using Azure Data Factory, enabling a new analytical web application for dealers that seamlessly processed <strong>100 GB of daily data</strong>.</li><li>Architected the <strong>migration of enterprise data from Hive tables</strong> into an Azure Cosmos DB application database, <strong>reducing query retrieval times by 40%</strong>.</li><li>Collaborated directly with <strong>Business Analysts and Backend Developers</strong> to define business requirements and structure optimized JSON schemas for Cosmos DB.</li></ul></div>`
  },
  abb: {
    ...
    html: `<div class="detail-block"><div class="detail-meta-row"><span class="detail-badge primary">Azure IoT</span><span class="detail-badge primary">PySpark</span><span class="detail-badge primary">ML Models</span><span class="detail-badge">ActiveMQ</span><span class="detail-badge">Event Hub</span><span class="detail-badge">Cosmos DB</span><span class="detail-badge">MongoDB</span><span class="detail-badge">Rundeck</span><span class="detail-badge">Azure Pipelines</span><span class="detail-badge success">Python APIs</span></div><div class="nb-cell"><div class="nb-cell__cmd">%spark</div><div class="nb-cell__code"><span class="nb-comment">// Real-time IoT streaming pipeline</span><br>df_stream = spark.readStream \\<br>  .format("eventhubs") \\<br>  .options(**ehConf) \\<br>  .load()<br><br>df_stream.writeStream \\<br>  .format("delta") \\<br>  .outputMode("append") \\<br>  .option("checkpointLocation", "s3://datalake/checkpoints/iot/") \\<br>  .table("delta_bronze.iot_telemetry")</div></div><ul class="detail-list"><li>Built a <strong>real-time data pipeline</strong> across Azure cloud and on-premises deployments to process <strong>semi-structured IoT telemetry data</strong> from ActiveMQ and Event Hub.</li><li>Designed a framework to <strong>deploy Machine Learning models on unstructured data</strong> (text, images, and videos), routing predictive results to Cosmos DB and MongoDB for downstream analytics.</li><li>Developed <strong>automated PySpark batch jobs</strong> to clean, transform, and migrate data from the data lake to the curated store, scheduling executions via <strong>Rundeck, Azure Pipelines, and Azure Data Factory</strong>.</li><li>Created <strong>custom Python APIs</strong> facilitating CRUD operations across various target data systems, improving data accessibility for the engineering team and saving an estimated <strong>15 hours of manual effort per week</strong>.</li></ul></div>`
  },
  infosys: {
    title: 'Data Engineer', subtitle: 'Infosys · Clients: PwC, Exelon · Mar 2017 – Feb 2020', sparkMeta: 'Format: Parquet on HDFS · Schedule: Cron · Testing: Selenium',
    html: `<div class="detail-block"><div class="detail-meta-row"><span class="detail-badge primary">PySpark</span><span class="detail-badge primary">HDFS</span><span class="detail-badge">Parquet</span><span class="detail-badge">Python</span><span class="detail-badge">Selenium</span><span class="detail-badge">Oracle DB</span><span class="detail-badge">MongoDB</span><span class="detail-badge">MySQL</span><span class="detail-badge">VSTS</span></div><div class="nb-cell"><div class="nb-cell__cmd">%spark</div><div class="nb-cell__code"><span class="nb-comment">// Multi-format ingestion → Parquet on HDFS</span><br>df = spark.read \\<br>  .format("csv").option("header", "true").load(path_csv)<br>  .unionByName(spark.read.json(path_json))<br>  .unionByName(spark.read.format("jdbc").option("url", mysql_url).load())<br><br>df.write.format("parquet") \\<br>  .mode("overwrite") \\<br>  .partitionBy("date") \\<br>  .save("hdfs://datalake/curated/")</div></div><ul class="detail-list"><li>Developed <strong>PySpark-based ETL workflows</strong> to migrate diverse data formats (<strong>CSV, JSON, MySQL</strong>) into an on-premises <strong>HDFS environment in Parquet format</strong>.</li><li>Optimized data delivery by executing <strong>query transformations and creating tables</strong> on top of HDFS files to push data to the curated layer, scheduled reliably using <strong>Cron</strong>.</li><li>Automated <strong>data API testing</strong> by writing robust test scripts using <strong>Python and Selenium</strong>, successfully validating data loads from Oracle DB to MongoDB.</li><li>Assisted in the <strong>architectural design of the data warehouse schema</strong> and actively resolved bugs via the <strong>VSTS DevOps platform</strong>.</li></ul></div>`
  },
  skills: {
